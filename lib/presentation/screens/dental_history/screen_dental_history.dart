...
import 'package:tooth_note/presentation/screens/dental_history/screen_add_dental_history.dart';
...
class DentalHistoryScreen extends ConsumerWidget {
    ...
    return Scaffold(
      ...
      floatingActionButton: InkWell(
        onTap: () {
          ...
          Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => const AddDentalHistoryScreen(),
              ));
        },
        child: Container(
      ...
      body: BlocBuilder<PatientBloc, PatientState>(
        builder: (context, patientstate) {
          ...
          if (patient == null ||
              patient.pastDentalHistory == null ||
              patient.pastDentalHistory!.pastDentalHistory.isEmpty) {
            return  Center(
              child: Image.asset(
          ...
          return ListView.builder(
            itemBuilder: (context, index) {
              if (index == patient!.pastDentalHistory!.pastDentalHistory.length) {
                return SizedBox(
            ...
            itemCount: patient.pastDentalHistory!.pastDentalHistory.length + 1,
          );
        },
      ),
    );
...
class EachContainerBox extends ConsumerWidget {
  ...
  Widget build(BuildContext context,WidgetRef ref) {
    DateTime? date;
    if (patient!.pastDentalHistory!.pastDentalHistory[index]['date'] is Timestamp) {
      date = patient!.pastDentalHistory!.pastDentalHistory[index]['date'].toDate();
    }
    return InkWell(
      onLongPress: () {
      ...
      onTap: () {
          Navigator.push(
            ...
            data: patient!.pastDentalHistory!.pastDentalHistory[index]['value'],
            onTap: () {
               addTime(date??DateTime.now(), ref);
             Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => AddDentalHistoryScreen(
                isToEdit: true,
                index: index,
                text: patient!.pastDentalHistory!.pastDentalHistory[index]['value'],
              ),
            ));
            }),
            ));
        ...
        child: Row(   mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            SizedBox(
              width: size.width*0.5,
              child: Text(
                patient!.pastDentalHistory!.pastDentalHistory[index]['value'],
                style: GoogleFonts.poppins(
                  textStyle: TextStyle(
                    letterSpacing: .5,
    ...
    if (value != null) {
      if (value == 1) {
        alertBox(context, 'Delete', () {
          ...
          PastDentalHistoryDTO? dentalHistoryDTO = patient.pastDentalHistory;
          if (dentalHistoryDTO != null) {
            dentalHistoryList = dentalHistoryDTO.pastDentalHistory;
          }
          dentalHistoryList.removeAt(index);
          ...
          BlocProvider.of<PatientBloc>(context).add(PatientEvent.addPatient(
            ...
            pastDentalHistory: newDentalHistoryDTO,
            patientId: patient.patientId,
